...
public class Clusters {
    ...
    private static Clusters cache = null;
    ...
    Map<File, NbModule> modulesByDir = new HashMap<>();
            ...
            for (String propName : props.stringPropertyNames()) {
                if (propName.startsWith(CLUSTER_PREFIX) && propName.endsWith(DIR_SUFFIX)) {
                    ...
                    for (String module : modules) {
                        NbModule nbm = ret.loadModule(new File(cluster.getDir(), module.trim()), cluster);
                        if (nbm != null) {
    ...
    public NbModule loadModule(File projectDir, NbCluster cluster) {
        NbModule ret = modulesByDir.get(projectDir);
        if (ret == null) {
            File projectXml = new File(projectDir, "nbproject/project.xml");
            if (projectXml.isFile()) {
                ret = new ProjectParser().parseProjectXML(projectXml, cluster);
                modulesByDir.put(projectDir, ret);
            }
        }
        return ret;
    }
